    ...
    class storage:
        class Client:
            def __getattribute__(self, _):
                raise ImportError("GCS upload not supported")
        def __getattribute__(self, _):
            raise ImportError("GCS upload not supported")
...
LABELS_FILE = 'LOC_val_solution.csv'

TRAINING_SHARDS = 1024
VALIDATION_SHARDS = 128

TRAINING_DIRECTORY = 'ILSVRC/Data/CLS-LOC/train'
VALIDATION_DIRECTORY = 'ILSVRC/Data/CLS-LOC/val'
...
def convert_to_tf_records(
  ...
  # validation_files = sorted(glob.glob(
  ...
  valid_df = pd.read_csv(os.path.join(raw_data_dir, "LOC_val_solution.csv")).rename(
      columns={"ImageId": "image_id"}
  )
  validation_synsets = valid_df["PredictionString"].str.split(" ", expand=True)[0]\
    .apply(lambda x: bytes(x, 'utf-8')).to_list()
  validation_files = valid_df["image_id"].apply(
      lambda x: os.path.join(raw_data_dir, VALIDATION_DIRECTORY, f"{x}.JPEG")
  ).to_list()
  ...
  training_records = _process_dataset(
      ...
      os.path.basename(TRAINING_DIRECTORY), TRAINING_SHARDS)
  ...
  validation_records = _process_dataset(
      ...
      os.path.basename(VALIDATION_DIRECTORY), VALIDATION_SHARDS)
