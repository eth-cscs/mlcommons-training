...
def main(argv: List[str]) -> None:
    ...
    if args.dataset_name == "criteo_1tb":
        all_files = [f"day_{i}" for i in range(24)]
        slurm_rank = int(os.environ.get('SLURM_PROCID', 0))
        slurm_world_size = int(os.environ.get('SLURM_NPROCS', 1))
        num_files_per_node = (24 + slurm_world_size - 1)//slurm_world_size
        in_files_l = all_files[num_files_per_node * slurm_rank:min(num_files_per_node * (slurm_rank + 1), 24)]
