...
from data.criteo import BinaryCriteoUtils
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        choices=['a', 'b'],
        help="Step to run (1: calculate sparse to contiguous mapping, 2: write updated files).",
    )
...
def main(argv: List[str]) -> None:
    ...
    if args.step == 'a':
        print(f"Calculating sparse to contiguous mapping for files {input_files}. Outputs will be saved to {output_dir}.")
        BinaryCriteoUtils.sparse_to_contiguous_efficient_mapping(
            input_files, output_dir, frequency_threshold=int(args.frequency_threshold)
        )
        print("Done calculating mapping.")
        return
    elif args.step == 'b':
        input_files = input_files[int(os.environ.get('SLURM_PROCID',0))::int(os.environ.get('SLURM_NTASKS',1))]
        print(f"Converting sparse to contiguous ids for files {input_files}. Outputs will be saved to {output_dir}.")
        BinaryCriteoUtils.sparse_to_contiguous_efficient_transform(
            input_files, output_dir, frequency_threshold=int(args.frequency_threshold)
        )
        print("Writing updated files.")
    else:
        raise ValueError("Invalid step value. Please set step to 1 or 2.")
