...
class BinaryCriteoUtils:
    ...
    @staticmethod
    def sparse_to_contiguous_efficient_mapping(
        in_files: List[str],
        output_dir: str,
        frequency_threshold: int = FREQUENCY_THRESHOLD,
        columns: int = CAT_FEATURE_COUNT,
        ...
        orig_in_files = in_files
        in_files = [os.path.join(output_dir, os.path.basename(f).split(".")[0] + '_transp.npy')
                    for f in in_files]

        def copy_transposed(orig_f, transp_f):
            path_manager = PathManagerFactory().get(path_manager_key)
            with path_manager.open(transp_f, 'wb') as transp_file:
                np.save(transp_file ,np.load(orig_f).transpose())
                print(f"Successfully transposed file: {orig_f}")

        if not all(os.path.exists(f) for f in in_files):
            joblib.Parallel(n_jobs=8)(joblib.delayed(copy_transposed)(orig_f, transp_f)
                                    for orig_f, transp_f in zip(orig_in_files, in_files))
            print(f"Successfully transposed all files: {orig_in_files}")
        else:
            print(f"All files already transposed: {in_files}")

        file_to_features: Dict[str, np.ndarray] = {}
        for f in in_files:
            name = os.path.basename(f).split(".")[0]
            file_to_features[name] = np.load(f, 'r') #.transpose()
            print(f"Successfully loaded file: {f}")

        # calculate range of sparse values
        sparse_min_max = joblib.Parallel(n_jobs=8)(joblib.delayed(lambda f: 
                                                                  (np.min(file_to_features[f]),
                                                                   np.max(file_to_features[f])))(f)
                                                   for f in file_to_features)
        sparse_min, sparse_max = zip(*sparse_min_max)
        sparse_range = max(int(v) for v in sparse_max) - min(int(v) for v in sparse_min)
        print(f"Successfully calculated min/max feature value for all files")

        def calc_sparse_to_contig(col):

            sparse_uniq_ordered = []
            sparse_counts_contig = np.zeros((sparse_range + 1,), dtype=int)

            for f, features in file_to_features.items():
                sparse_uniq, sparse_inds, sparse_counts = np.unique(features[col], return_index=True, return_counts=True)

                sparse_inds = np.argsort(sparse_inds)
                sparse_uniq = sparse_uniq[sparse_inds]
                sparse_counts = sparse_counts[sparse_inds]

                sparse_uniq_ordered.append(sparse_uniq[sparse_counts_contig[sparse_uniq] == 0])

                sparse_counts_contig[sparse_uniq] += sparse_counts
                print(f"Successfully processed frequency distribution for col {col} of file: {f}")

            sparse_uniq_ordered = np.concatenate(sparse_uniq_ordered)

            sparse_uniq_counts = sparse_counts_contig[sparse_uniq_ordered]
            sparse_uniq_counts[sparse_uniq_counts >= max(1,frequency_threshold)] = \
                np.arange(2, (sparse_uniq_counts >= max(1,frequency_threshold)).sum() + 2, dtype=int)

            if frequency_threshold > 1:
                sparse_counts_contig[(0 < sparse_counts_contig) & (sparse_counts_contig < frequency_threshold)] = 1

            sparse_counts_contig[sparse_uniq_ordered] = sparse_uniq_counts

            path_manager = PathManagerFactory().get(path_manager_key)
            with path_manager.open(os.path.join(output_dir, f"sparse_to_contig_col_{col}.npy"), 'wb') as f:
                np.save(f, sparse_counts_contig)

            print("Successfully saved file: ", os.path.join(output_dir, f"sparse_to_contig_col_{col}.npy"))

        joblib.Parallel(n_jobs=8)(joblib.delayed(calc_sparse_to_contig)(col)
                                  for col in range(columns))

        print(f"Successfully calculated sparse_to_contig for all columns")


    @staticmethod
    def sparse_to_contiguous_efficient_transform(
        in_files: str,
        output_dir: str,
        frequency_threshold: int = FREQUENCY_THRESHOLD,
        columns: int = CAT_FEATURE_COUNT,
        ...
        for f in in_files:
            name = os.path.join(output_dir, os.path.basename(f).split(".")[0] + '_transp.npy')
            features = np.load(name)  #.transpose()
            print(f"Successfully loaded file: {name}")

            for col in range(columns): 
                sparse_to_contig = np.load(os.path.join(output_dir, f"sparse_to_contig_col_{col}.npy"))
                features[col] = sparse_to_contig[features[col]]
                print(f"Successfully updated column {col} with contiguous ids.")

            path_manager = PathManagerFactory().get(path_manager_key)
            output_file = os.path.join(output_dir, os.path.basename(f).split(".")[0] + output_file_suffix)
            with path_manager.open(output_file, "wb") as fout:
                print(f"Writing file: {output_file}")
                # Transpose back the features when saving, as they were transposed when loading.
                np.save(fout, features.transpose())
            print(f"Successfully saved file {os.path.basename(output_file)} with contiguous ids.")


    @staticmethod
    def sparse_to_contiguous_efficient(
        in_files: List[str],
        output_dir: str,
        frequency_threshold: int = FREQUENCY_THRESHOLD,
        columns: int = CAT_FEATURE_COUNT,
        ...
        raise RuntimeError("This function is deprecated, use sparse_to_contiguous_efficient_mapping/_transform instead.")
        ...
        def copy_transposed(orig_f, transp_f):
            with open(transp_f, 'wb') as transp_file:
                np.save(transp_file ,np.load(orig_f).transpose())
                print(f"Successfully transposed file: {orig_f}")
        ...
        # TODO: maybe define this within every joblib function newly
        file_to_features: Dict[str, np.ndarray] = {}
        for f in in_files:
            name = os.path.basename(f).split(".")[0]
            file_to_features[name] = np.load(f, 'r+') #.transpose()
        ...
        def calc_sparse_to_contiguous_int_colwise(col):
        # # Iterate through each column in each file and map the sparse ids to contiguous ids.
        # for col in range(columns):
            print(f"Processing column: {col}")
            ...
            return col, sparse_to_contiguous_int

        sparse_to_contiguous_int_colwise = dict(
            joblib.Parallel(n_jobs=8)(joblib.delayed(calc_sparse_to_contiguous_int_colwise)(col)
                                       for col in range(columns))
        )
        print(f"Successfully calculated mapping sparse -> contiguous for all files")
        

        def update_and_write_file(f):
            features = file_to_features[f]
            for col in range(columns):
                for i, sparse in enumerate(file_to_features[f][col]):
                    features[col][i] = sparse_to_contiguous_int_colwise[col][sparse]
        ...
        joblib.Parallel(n_jobs=1)(joblib.delayed(update_and_write_file)(f)
                                   for f in file_to_features)
        print(f"Successfully wrote all files with contiguous ids.")
    ...
    def shuffle_efficient(
        ...
        if output_dir_full_set is not None:
            raise ValueError("Full data materialization currently not implemented")  # use np.memmap

        total_rows = sum(rows_per_day.values())
        print(f"Total rows in dataset to shuffle: {total_rows}")
        ...
        print("Shuffled permutation")

        bounds = np.cumsum([0] + [rows_per_day[d] for d in range(0, days - 1)])

        def write_shuffled_files(d, bounds, perm):
            source_days = np.searchsorted(bounds, perm, side='right') - 1

            shuffled_dense = np.zeros((bounds[d+1]-bounds[d], int_columns), dtype=np.float32)
            shuffled_sparse = np.zeros((bounds[d+1]-bounds[d], sparse_columns), dtype=np.float32)
            shuffled_labels = np.zeros((bounds[d+1]-bounds[d], 1), dtype=np.float32)

            for i in range(0, days - 1):
                print(f"Reading source day {i}")

                mask = source_days == i
                
                shuffled_dense[mask] = np.load(
                    os.path.join(input_dir_labels_and_dense, f"day_{i}_dense.npy"))[perm[mask] - bounds[i]]
                shuffled_sparse[mask] = np.load(
                    os.path.join(input_dir_sparse, f"day_{i}_sparse.npy"))[perm[mask] - bounds[i]]
                shuffled_labels[mask] = np.load(
                    os.path.join(input_dir_labels_and_dense, f"day_{i}_labels.npy"))[perm[mask] - bounds[i]]
            ...
            with path_manager.open(shuffled_dense_file, "wb") as fout:
                print(
                    f"Writing dense file: {shuffled_dense_file}"
                )
                np.save(fout, shuffled_dense)
            ...
            with path_manager.open(shuffled_sparse_file, "wb") as fout:
                print(
                    f"Writing sparse file: {shuffled_sparse_file}"
                )
                np.save(
                    fout,
                    shuffled_sparse.astype(np.int32),
                )
            ...
            with path_manager.open(shuffled_labels_file, "wb") as fout:
                print(
                    f"Writing labels file: {shuffled_labels_file}"
                )
                np.save(
                    fout,
                    shuffled_labels.astype(np.int32),
                )

        for d in range(0, days - 1):
            write_shuffled_files(d, bounds, permutation[bounds[d]:bounds[d+1]])

        # permutations = {d: permutation[bounds[d]:bounds[d+1]].copy() for d in range(0, days - 1)}
        # import joblib
        # joblib.Parallel(n_jobs=8)(joblib.delayed(write_shuffled_files)(d, bounds.copy(), permutations[d])
        #                           for d in range(0, days - 1))
        print(f"Successfully shuffled {days - 1} files")
